...
namespace LiteDB.Studio
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            this.tabSql.HandleCreated += this.TabSql_HandleCreated;
        ...
        private void TabSql_Selected(object sender, TabControlEventArgs e)
        {
            ...
            if (tabSql.SelectedIndex == tabSql.TabPages.Count - 1)
            {
                this.AddNewTab("");
            }
            else
        ...
        private void TabSql_DrawItem(object sender, DrawItemEventArgs e)
        {
            try
            {
                var tabPage = this.tabSql.TabPages[e.Index];
                var tabRect = this.tabSql.GetTabRect(e.Index);
                tabRect.Inflate(-2, -2);
                if (e.Index == this.tabSql.TabCount - 1) // Add button to the last TabPage only
                {
                    var addImage = new Bitmap(Resources.tab_add);
                ...
                {
                    var closeImage = new Bitmap(Resources.tab_close);
        ...
        private void TabSql_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < tabControl.TabPages.Count - 1; i++)
                {
        ...
        private void TabSql_HandleCreated(object sender, EventArgs e)
        {
            //SendMessage(this.tabSql.Handle, TCM_SETMINTABWIDTH, IntPtr.Zero, (IntPtr)16);
        }
    }
}
